...
(function() {
    ...
    var pathTemplate = '<labels   class="js-reviewed-toggle ml-2 mr-1 px-2 py-1 rounded-2 f6 text-normal d-flex flex-items-center border js-reviewed-file color-bg-accent color-border-accent"   style="cursor: pointer" >   <input class="mr-1 js-reviewed-checkbox" type="checkbox" value="{{Path}}" checked="checked"   />   {{Path}} </label> ';
    ...
    if (!gdf) {
        ...
        function appendPathFilters() {
            ...
            getRootPaths().forEach(path => {
                list.append(document.createRange().createContextualFragment(pathTemplate.replaceAll("{{Path}}", path)));
            });
        ...
        function showTableOfContentsEntry(pattern) {
            [ ...document.querySelectorAll('#toc a[href^="#diff"]') ].filter(el => el.textContent.match(pattern)).map(el => el.closest("li")).forEach(show);
        }
        ...
        function showDiffEntry(pattern) {
            [ ...document.querySelectorAll(`[data-tagsearch-path]`) ].filter(el => el.attributes["data-tagsearch-path"]?.value.match(pattern)).forEach(show);
        }
        ...
        function getRootPaths() {
            return [ ...new Set([ ...document.querySelectorAll("[data-tagsearch-path]") ].map(el => el.attributes["data-tagsearch-path"].value).map(path => path.split("/")[0])) ].map(path => path + "/*");
        }
        function hide(el) {
